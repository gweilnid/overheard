...
colors:
  ...
  text-muted: "#6A6A74"
  text-light: "#A2A2AC"
  accent-light: "rgba(245, 64, 29, 0.08)"
  accent-medium: "rgba(245, 64, 29, 0.15)"
  border: "#EBEBF0"
  card-bg: "#FFFFFF"
  positive: "#059669"
  negative: "#dc2626"
...
- **Fonts** — already set to **DM Mono** (display) / **DM Sans** (body); ignore any preset font name lingering in prose.
...
## Brand adaptation — hand adjustments after build-frame

`build-frame.mjs` mapped the preset's neutral text roles onto brand reds
(`text-muted: #DC2626`), which would have set every line of body copy in red.
Five keys were repainted from the product's own live tokens so the video's
neutrals match the captured dashboard exactly:

| key         | script output           | corrected | source                 |
| ----------- | ----------------------- | --------- | ---------------------- |
| text-muted  | `#DC2626`               | `#6A6A74` | board `--ink2`         |
| text-light  | `#9A9A9A`               | `#A2A2AC` | board `--ink3`         |
| bg          | `#FFFFFF`               | `#F1F1F4` | board `--page`         |
| card-bg     | `rgba(245,64,29,0.04)`  | `#FFFFFF` | board `--tile`         |
| border      | `rgba(245,64,29,0.2)`   | `#EBEBF0` | board `--hair`         |

`primary` (`#F5401D`, board `--hot`) stays the single accent, exactly as the
preset intends. Recreated Telegram chrome is the one licensed exception: it uses
Telegram's own colors so the app reads as itself, never as our brand.
